...
```{r requiredCourse, echo=TRUE}
query <- 'MATCH (c:Course)-[:Part_Of{type:"required"}]->(m:Minor) RETURN m.name, count(c), sum(toInt(c.minCredits))'
gq <- cypher(gdb,query) %>% as.data.frame()
names(gq) <- c("Minor","Number.Classes", "Credit.min")

kable(arrange(gq, Minor, desc(Number.Classes)), caption = "Number of courses required for each minor")
...
```{r optGroups, echo=TRUE}
query <- 'MATCH (c:Course)-[p:Part_Of{type:"option"}]->(m:Minor) RETURN m.name, p.cluster'
gq <- cypher(gdb,query) %>% as.data.frame()
...
```{r optCourse, echo=TRUE}
query <- 'MATCH (c:Course)-[p:Part_Of{type:"option"}]->(m:Minor) RETURN m.name, p.cluster, count(c), sum(toInt(c.minCredits))'

gq <- cypher(gdb,query) %>% as.data.frame()
names(gq) <- c("Minor", "Option.Group", "Number.Classes", "Credit.min")

kable(arrange(gq, Minor, Option.Group, desc(Number.Classes)), caption = "Number of courses for each option group of each minor")
```