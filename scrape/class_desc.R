...
get_descriptions <- function(class_url){
  ...
  course_pre <- str_split(course_desc_text, 'Prerequisite[s]?:|Prereq.:|Prerequisite ', n=2, simplify = TRUE)[,2]
  course_coreq <- str_split(course_desc_text, 'Corequisite[s]?:', n=2, simplify = TRUE)[,2]
  #credits
